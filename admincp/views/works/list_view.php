    ...
    <table border="0">
      <tbody>
         <tr>
            ...
            <td> Quản lý nội dung / Hãng Sản Xuất </td>
         </tr>
      </tbody>
    </table>
    </div>
...
<form action='<?=BASE_URL_ADMIN;?>works/list' method='post' name="rowsForm" id="rowsForm">
<table class="view">
	<tr>
		...
		<th>Sắp Xếp</th>
		<th width="50"><?=G_LOCK; ?></th>
		<th colspan='2'><?=G_ACTION;?></th>
	</tr>
	<?php if(empty($data['info'])){ ?>

	<tr>
		<td colspan='15' class='emptydata'><?=G_EMPTYDATA;?></td>
	</tr>
	<?php } else { foreach($data['info'] as $item) { ?>
    <tr>
        <td align = 'center'><input type="checkbox" id='check_list' name="check_list[]" value="<?=$item['Id'];?>"><br></td>
        <td><?=$item['Id']?></td>
        <td align="left"><a href='<?=BASE_URL_ADMIN."works/edit/".$item['Id'];?>' title='Sửa'><?=$item['title_vn'];?></a></td>
        <td><img src='<?=BASE_URL."data/Works/".$item['images'];?>' width='60'></td>
        <td align='center'>
        <?php 
            if($item['home'] == "1"){
